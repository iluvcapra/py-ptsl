...
def timecode_info(session_rate: 'SessionTimeCodeRate') -> Tuple[Fraction, bool]:
    """
    For the given :class:`~ptsl.PTSL_pb2.SessionTimeCodeRate` enumeration value,
    returns a library-agnostic description that can be used for time arithmetic.

    The first element in the return :class:`tuple` is the duration of a frame in the given 
    ``session_rate`` as a fractional number of seconds. The second element is a `bool`
    which is `True` if the session_rate is drop-frame.
...
def pull_rate_info(rate_pull: 'SessionRatePull') -> Tuple[int, int]:
    ...
    4% pull value (as when converting 24fps and 25fps playback), and ``retval[1]``
    can be multiplied by 0.1% to give the 0.1% pull value (as when converting
    NTSC to integral frame rates).  
