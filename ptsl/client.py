...
class Auditor:
    ...
    def emit(self, message):
        if self.enabled:
            ...
            print("%04i%s %s" % (self.command_sn, time_str, message), file=self.output_stream)

    def run_called(self, command: pt.CommandId):
        self.emit("Started Command %s (%i)" % (pt.CommandId.Name(command), command))
...
class Client:
    ...
    def _handle_completed_response(self, operation, response):
        ...
        if len(response.response_body_json) > 0 and p is not None:
            self.auditor.response_json_before_cleanup(response.response_body_json)
